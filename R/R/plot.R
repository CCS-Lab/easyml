...
plot_betas <- function(betas) {
  g <- 
    ggplot2::ggplot(betas, ggplot2::aes(x = predictor, y = mean, colour = dotColor)) +
...
plot_roc_curve <- function(y_true, y_pred) {
  ...
  g <- ggplot2::ggplot(df, ggplot2::aes(x = one_minus_specificities, y = sensitivities)) +
    ggplot2::geom_path(alpha = 1, size = 1) +
    ggplot2::geom_segment(ggplot2::aes(x = 0, y = 0, xend = 1, yend = 1) , linetype = "dashed") + 
    ggplot2::annotate("text", label = auc_label, x = 0.6, y = 0.10, size = 8, colour = "black") + 
