...
#' @return TO BE EDITED.
#' @export
svm_fit_model_gaussian <- function(X, y, ...) {
  X <- as.matrix(X)
...
#' @return TO BE EDITED.
#' @export
svm_fit_model_binomial <- function(X, y, ...) {
  X <- as.matrix(X)
  y <- factor(y)
...
#' @return TO BE EDITED.
#' @export
svm_predict_model <- function(results, newx) {
...
#' @return TO BE EDITED.
#' @export
easy_svm <- function(.data, dependent_variable, family = "gaussian", 
  ...
  if (family == "gaussian") {
    ...
    predictions <- replicate_predictions(svm_fit_model_gaussian, 
                                         svm_predict_model, 
                                         preprocessor, 
    ...
    mses <- replicate_mses(svm_fit_model_gaussian, 
                           svm_predict_model, sampler, preprocessor, X, y, 
                           categorical_variables = categorical_variables, 
    ...
    # Replicate predictions
    predictions <- replicate_predictions(svm_fit_model_binomial, 
                                         svm_predict_model, 
                                         preprocessor, 
                                         X_train, y_train, X_test, 
                                         categorical_variables = categorical_variables, 
    ...
    aucs <- replicate_aucs(svm_fit_model_binomial, 
                           svm_predict_model, 
